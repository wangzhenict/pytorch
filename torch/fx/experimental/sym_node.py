...
def _optimized_add(
    ...
    no other optimizations is needed. Figuring out the order can be done in Log(n) comparisons when its known that the input
    is already sorted. Returns a tuple of (1) a boolean that indicates whether the output is a summation of unique symbols,
...
def _make_node_magic(method, func):
    ...
    def binary_magic_impl(self, other):
        ...
        if optimized_summation:
            # the add expression in sympy has empty __slots__, this made not possible to attach attributes
            # to it. Furthermore, we can't use it as weak dictionary key also! so instead we attach the
            # attribute here to the SymNode.
