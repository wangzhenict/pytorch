...
class TestOptimizer(TestCase):
    ...
    @skipIfNoXNNPACK
    def test_conv1d_with_same_padding(self):
        class Conv1dTestModuleSamePadding(torch.nn.Module):
            def __init__(self):
                super().__init__()

            def forward(self, input, weight):
                x = torch.nn.functional.conv1d(input, weight, padding='same')
                return x
        test_module = Conv1dTestModuleSamePadding()
        test_module(torch.rand(1, 5, 30), torch.rand(6, 5, 7))
        test_module.eval()
        scripted_module = torch.jit.script(test_module, (torch.rand(1, 6, 30), torch.rand(6, 5, 7)))
        opt_module = optimize_for_mobile(scripted_module)
        input = (torch.rand(1, 5, 30), torch.rand(6, 5, 7))
